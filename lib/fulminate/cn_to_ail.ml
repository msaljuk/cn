...
let gen_bool_while_loop sym bt start_expr while_cond ?(if_cond_opt = None) (bs, ss, l, e) =
  (*
     Input:
...
let cn_to_ail_const const basetype 
  : (CF.GenTypes.genTypeCategory A.expression_ * CnL.lua_expr * bool)
=
  match RC.get_runtime() with
    | RC.C ->
      ...
      (ail_const, CnL.get_empty_lua_expr, is_unit)
    | RC.Lua ->
      let lua_expr, is_unit = CnL.cn_to_lua_const const basetype in
      (*let open Lua.Pp_lua in
      print_endline (pp_expr lua_expr);*)
      (ail_null, lua_expr, is_unit)
  ...
  | Return ->
    (match RC.get_runtime() with
      ...
      | RC.Lua ->
        ...
        let l'' = CnL.push_stmt_to_exec (l', return_stmt) in
        (b, s, l''))
  | AssignVar x ->
...
let dest
  : type a.
    ...
    * CF.GenTypes.genTypeCategory A.expression ->
    a
  =
  fun d spec_mode_opt (b, s, l, e) -> dest_with_unit_check d spec_mode_opt (b, s, l, e, false)
...
let rec cn_to_ail_expr_aux
  ...
  match term_ with
  | Const const ->
    let ail_expr, lua_cn_expr, is_unit = cn_to_ail_const const basetype in
    let l = CnL.push_expr_to_exec (CnL.get_empty_lua_cn_exec, lua_cn_expr) in
    dest_with_unit_check d spec_mode_opt ([], [], l, mk_expr ail_expr, is_unit)
  | Sym sym ->
    (match RC.get_runtime() with
      | RC.C ->
        let sym =
        ...
        let ail_expr_, _ =
          ...
          | Some (sym2, cn_const) ->
            if CF.Symbol.equal_sym sym sym2 then (
              let ail_const, lua_expr, _ = cn_to_ail_const cn_const basetype in
              ail_const, lua_expr)
            else
              (A.AilEident sym, CnL.get_empty_lua_expr)
          | None -> (A.AilEident sym, CnL.get_empty_lua_expr)
...
let cn_to_ail_post
  ...
  =
  let bs, ss, ls = cn_to_ail_post_aux filename dts globals preds (Some Post) t in
...
let rec cn_to_ail_cnprog_ghost_arg filename dts globals spec_mode_opt i = function
  | Cnprog.Let (_loc, (name, { ct; pointer }), prog) ->
    let b1, s, l, e = cn_to_ail_expr filename dts globals spec_mode_opt pointer PassBack in
    let cn_ptr_deref_sym = Sym.fresh "cn_pointer_deref" in
    let ctype_sym =
      Sym.fresh
        (Pp.plain
  ...
  | Pure (loc, ghost_it) ->
    ...
    in
    (bs, upd_s @ ss @ [ A.AilSexpr add_to_ghost_array_call ] @ pop_s, ls)
...
let rec cn_to_ail_lat_internal_loop
  ...
  | LAT.Define ((name, it), _info, lat) ->
    ...
    let binding = create_binding name ctype in
    let decl = A.(AilSdeclaration [ (name, None) ]) in
    let b1, s1, l1 = cn_to_ail_expr filename dts globals spec_mode_opt it (AssignVar name) in
    let b2, s2, l2 =
...
let cn_to_ail_assume_pre filename dts sym args globals preds lat
  ...
  in
  (* Generate function *)
  let bs', ss', _ = cn_to_ail_assume_lat_2 filename dts (Some sym) globals preds None lat in
