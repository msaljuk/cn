...
let get_cn_stack_depth_sym = Sym.fresh "lua_cn_get_stack_depth"
...
let c_add_ownership_fn_sym = Sym.fresh "lua_cn_ghost_add"

let c_remove_ownership_fn_sym = Sym.fresh "lua_cn_ghost_remove"
...
let generate_c_local_ownership_entry_bs_and_ss (sym, ctype) =
  ...
  @note saljuk: clearing this for now since we don't need cn pointer generation in c.
   will likely reuse some parts of this for actual lua gen code
  *)
  if false then
