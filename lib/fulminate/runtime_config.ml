...
let runtime_to_string = function C -> "C" | Lua -> "Lua"
...
let set_runtime r =
  ...
  | Some curr ->
    failwith
      ("Cannot set runtime to "
       ^ runtime_to_string r
       ^ ". \n        It has already been initialized to "
       ^ runtime_to_string curr)


let get_runtime () =
  match !runtime with Some r -> r | None -> failwith "No valid runtime set."
